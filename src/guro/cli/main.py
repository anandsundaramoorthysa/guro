...
@cli.command(name='benchmark', help="🔥 Run system benchmarks.")
...
@cli.command(name='heatmap', help="🌡️ Visualize system temperature with a heatmap.")
@click.option('--interval', '-i', default=1.0, help='Update interval in seconds (default: 1.0).')
@click.option('--duration', '-d', default=None, type=int, help='Duration to run in seconds (default: None, which means no limit).')
def heatmap(interval: float, duration: Optional[int]):
    """Visualize system temperature with a heatmap."""
    heatmap_monitor = SystemHeatmap()
    heatmap_monitor.run(interval=interval, duration=duration)
